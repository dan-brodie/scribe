...
- [x] Mic captured via `AVAudioEngine` at 16 kHz mono Float32
- [x] System audio captured via Core Audio process tap (macOS 14.4+), same format, separate file
- [x] Both files written to `~/Library/Application Support/Scribe/recordings/<eventID>/mic.caf` and `system.caf`
- [x] Audio flushed to disk every ≤5 s (crash loses <5 s of audio)
- [x] Recording starts automatically at meeting start (configurable lead: 0–2 min)
- [x] Recording stops on: calendar end + grace period (default 2 min), 90 s sustained silence post-schedule, or manual stop
- [x] Manual "Record now" works without a calendar event
- [x] AirPods connect/disconnect mid-recording is handled without dropping the session
- [x] Zero-energy system channel (device mismatch) detected after 30 s → user warned via notification
- [x] `CaptureService` exposes `startRecording(for:)` and `stopRecording()` async methods
- [x] ScreenCaptureKit fallback path compiles behind `SCREENCAPTURE_FALLBACK` flag (not default)
- [x] Unit tests: stop-condition logic (silence detection, grace period) with mock audio input
